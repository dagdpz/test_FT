...
if plot_separately == 1;
    ...
    for c = 1:numel(names);
        ...
        FigName_Long1=sprintf('%s value with locking to %d Hz LFP with bursts', names{c}, lfp_freq(spikePhaseFreq)); % overall title of the figure
...
if plot_compared_ppc == 1;
    ...
    FigName_Long1=sprintf('PPC value with locking to %d Hz LFP with bursts', lfp_freq(spikePhaseFreq)); % overall title of the figure
    set(0,'DefaultTextInterpreter','none');
    wanted_papersize=[40 25];
    h1=figure('outerposition',[0 0 1900 1200],'name', FigName_Long1);
    figure(h1)
    ...
    for el = 1:numel(spikeLockProbs);
        figure(1); hold on
        plot(spikeRates, ppc0(el, :), 'o-', 'Color', rand_rgb(el,:), 'MarkerFaceColor', rand_rgb(el,:));
        plot(spikeRates, ppc1(el, :), 'o--', 'Color', rand_rgb(el,:), 'MarkerFaceColor', rand_rgb(el,:))
